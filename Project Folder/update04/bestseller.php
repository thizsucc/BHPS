...
// Fetch bestseller books from JSON file
...
if (count($bestseller_ids) > 0) {
    ...
    if (!empty($search)) {
        $sql = "SELECT * FROM book WHERE BookID IN ($in) AND (Name LIKE ? OR Author LIKE ? OR BookID LIKE ? OR Category LIKE ?)";
        $stmt = $conn->prepare($sql);
        $search_param = "%$search%";
        $params = array_merge($bestseller_ids, [$search_param, $search_param, $search_param, $search_param]);
        $stmt->bind_param(str_repeat('s', count($bestseller_ids)) . 'ssss', ...$params);
    } else {
        $sql = "SELECT * FROM book WHERE BookID IN ($in)";
        $stmt = $conn->prepare($sql);
...
<head>
    ...
    <title>Bestseller Books - Book Heaven</title>
...
<body class="bg-gray-50 font-sans">
                    ...
                    <div class="relative w-full max-w-xl">
                        <form method="GET" class="search-box flex items-center border rounded-full overflow-hidden">
                            <input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>"
                                placeholder="Search bestsellers, authors..."
            ...
            <aside class="md:w-1/4">
                ...
                <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
                    ...
                    <p class="text-gray-600 mb-4">
                        Discover our most popular books loved by readers everywhere. Our bestsellers collection features
                        the books that have captured the hearts and minds of our customers.
                    </p>
                    <p class="text-gray-600">
                        From thrilling page-turners to heartwarming stories, these books have proven to be favorites
                        time and time again. Find your next great read among our curated selection of bestsellers.
                    </p>
                </div>
            ...
            <div class="md:w-3/4">
                <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
                    <div class="flex flex-col md:flex-row md:items-center justify-between mb-6">
                        <h1 class="text-2xl font-bold">Bestseller Books</h1>
                    ...
                    <?php if (!empty($search)): ?>
                        <div class="mb-4 p-4 bg-blue-50 rounded-lg">
                            <p class="text-blue-800">
                                ...
                                <a href="bestseller.php" class="text-blue-600 hover:underline ml-4">Clear search</a>
                            </p>
                        </div>
                    <?php endif; ?>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        ?>
                            <div class="book-card bg-white rounded-lg shadow-sm overflow-hidden"
                                ...
                                <div class="relative">
                                    ...
                                    <?php if ($is_promotion): ?>
                                        <span class="absolute top-2 left-16 bg-red-600 text-white text-xs px-2 py-1 rounded">PROMOTION</span>
                                    <?php endif; ?>
                                    <?php if (in_array($book['BookID'], $latest_book_ids)): ?>
                                        <span class="absolute top-2 <?php echo $is_promotion ? 'left-32' : 'left-16'; ?> bg-red-600 text-white text-xs px-2 py-1 rounded">NEW</span>
                                    <?php endif; ?>
                        ...
                        <?php if (empty($bestseller_books)): ?>
                            <div class="col-span-3 text-center py-8">
                                ...
                                <p class="text-gray-500">No bestseller books found<?php echo !empty($search) ? ' matching your search' : ''; ?>.</p>
                                <?php if (!empty($search)): ?>
                                    <a href="bestseller.php" class="text-blue-600 hover:underline mt-2 inline-block">Browse all bestsellers</a>
                                <?php endif; ?>
                            </div>
                        <?php endif; ?>
                    </div>
    ...
    <script>
        ...
        `;
            ...
            cartShippingFee.textContent = `RM${finalShippingFee.toFixed(2)}`;
