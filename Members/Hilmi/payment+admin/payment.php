...
function generateOrderID($conn) {
    // Get the last OrderID from the database to maintain sequence
    $result = $conn->query("SELECT MAX(OrderID) as last_id FROM `order`");
    $row = $result->fetch_assoc();
    $last_id = $row['last_id'] ?? 1000; // Start from 1000 if no orders exist
    
    // Generate a random number between 100000 and 999999
    $random_id = mt_rand(100000, 999999);
    
    // Ensure the new ID is greater than the last ID to maintain sequence
    while ($random_id <= $last_id) {
        $random_id = mt_rand(100000, 999999);
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['pay_now'])) {
  ...
  // FIXED: Get user ID from session - ensure it's the correct field name
  $user_id = null;
  ...
  if (!$user_id) {
      ...
      // FIXED: Ensure user_id is properly cast to integer
      $user_id = (int)$user_id;
      ...
      if (empty($address)) {
        ...
        // Generate a single OrderID for this entire order
        ...
        foreach ($phpCart as $index => $item) {
          ...
          // If no BookID found, try to get it from database using title
          if (!$book_id) {
          ...
          // DEBUG: Log the data being inserted with user ID
          error_log("Inserting order - OrderID: $order_id, User_ID: $user_id, Address: $address, BookID: $book_id, Quantity: $qty, Total: $total_amount");
          
          // FIXED: Use User_ID (with underscore) to match database column name
          $stmt = $conn->prepare("INSERT INTO `order` (OrderID, BookID, order_date, address, Quantity, User_ID, Status, Total_Amount) VALUES (?, ?, ?, ?, ?, ?, 'Processing', ?)");
          
          // Debug the parameters before binding
          error_log("Parameters - OrderID: $order_id (int), BookID: $book_id (string), Date: $order_date, Address: $address, Qty: $qty (int), User_ID: $user_id (int), Total: $total_amount (float)");
          
          // FIXED: Correct parameter types - User_ID should be integer
          $stmt->bind_param("isssiid", $order_id, $book_id, $order_date, $address, $qty, $user_id, $total_amount);
          
          if ($stmt->execute()) {
            if (!in_array($order_id, $order_ids)) {
              $order_ids[] = $order_id;
            }
            error_log("Successfully inserted order with ID: $order_id for user: $user_id");
          } else {
            $all_success = false;
...
$shipping = count($phpCart) > 0 ? 5.00 : 0.00;
    ...
    <main class="container mx-auto px-4 py-8">
        <div class="max-w-6xl mx-auto">
            ...
            <div class="debug-info">
                ...
                Session userID: <?php echo isset($_SESSION['userID']) ? $_SESSION['userID'] : 'NOT SET'; ?><br>
                Session UserID: <?php echo isset($_SESSION['UserID']) ? $_SESSION['UserID'] : 'NOT SET'; ?><br>
                Session loggedin: <?php echo isset($_SESSION['loggedin']) ? ($_SESSION['loggedin'] ? 'TRUE' : 'FALSE') : 'NOT SET'; ?><br>
                Cart items: <?php echo count($phpCart); ?><br>
                User ID to be used: <?php echo isset($user_id) ? $user_id : 'NOT SET'; ?><br>
                                    ...
                                    <div id="bank-details" class="mt-4 pl-6 hidden">
                                        <div>
                                            ...
                                            <select id="bank_name" name="bank_name" required
                                                ...
                                                <option value="https://www.cimbclicks.com.my/">CIMB Clicks</option>
                                                <option value="https://www.bankislam.com/">Bank Islam</option>
                                                <option value="https://www.agrobank.com.my/">Agrobank</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
...
<?php if ($payment_success): ?>
                        <div class="success-message mt-6">
                            ...
                            <?php if (isset($_SESSION['last_order_ids'])): ?>
                                <p class="font-medium mb-4">Order ID: <?php echo implode(', ', $_SESSION['last_order_ids']); ?></p>
    ...
    <script>
        ...
        window.onclick = function(event) {
            if (!event.target.matches('button') && !event.target.closest('button')) {
                ...
                for (let i = 0; i < userDropdowns.length; i++) {
                    userDropdowns[i].classList.remove('show');
                }
        ...
        // Store selected bank URL before form submission
        document.querySelector('form').addEventListener('submit', function(e) {
            const selectedBank = document.getElementById('bank_name');
            if (selectedBank && selectedBank.value) {
                sessionStorage.setItem('selectedBankURL', selectedBank.value);
            }
        });
    </script>
    ...
    <div class="bg-green-100 text-green-700 p-4 rounded mb-4 text-center">
        ...
        <p>Your Order ID(s): <?= implode(', ', $order_ids) ?></p>
    </div>

    <script>
        // Redirect to the selected bank in a new tab after successful order
        const bankURL = sessionStorage.getItem('selectedBankURL');
        if (bankURL) {
            window.open(bankURL, '_blank'); // new tab
            sessionStorage.removeItem('selectedBankURL');
        }
    </script>
    <?php endif; ?>
